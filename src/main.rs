...
pub enum Message {
   ...
   UsageQuotaUpdated(String), // New: Message to display AI usage quota
...
pub enum BlockMessage {
    ...
    FetchUsageQuota, // New: Fetch AI usage quota
}
...
impl Application for NeoTerm {
    ...
    fn new(_flags: ()) -> (Self, Command<Message>) {
        ...
        // Initialize shared resources for AIContext
        let command_manager_for_ai = Arc::new(CommandManager::new(mpsc::channel(100).0)); // Dummy sender
        let virtual_file_system_for_ai = Arc::new(VirtualFileSystem::new());
        let watcher_for_ai = Arc::new(Watcher::new(mpsc::channel(100).0)); // Dummy sender

        // Initialize AI Assistant first
        let ai_assistant = Arc::new(RwLock::new(Assistant::new(
            command_manager_for_ai.clone(),
            virtual_file_system_for_ai.clone(),
            watcher_for_ai.clone(),
            ...
            config.preferences.redact_sensitive_info, // Pass redact_sensitive_info
            config.preferences.local_only_ai_mode, // Pass local_only_ai_mode
        ).unwrap()));
...
impl NeoTerm {
    fn create_toolbar(&self) -> Element<Message> {
        ...
        let usage_button = button(text("📊 Usage"))
            .on_press(Message::BlockAction("".to_string(), BlockMessage::FetchUsageQuota)); // Block ID is dummy for this action
...
impl App {
    pub async fn new() -> Result<(App, mpsc::Sender<AppMessage>)> {
        ...
        let ai_assistant = Arc::new(RwLock::new(Assistant::new(
            command_manager.clone(), // Pass command_manager
            Arc::new(VirtualFileSystem::new()), // Pass VirtualFileSystem
            watcher.clone(), // Pass watcher
            ...
            preferences.redact_sensitive_info, // Pass redact_sensitive_info
            preferences.local_only_ai_mode, // Pass local_only_ai_mode
        )?));
