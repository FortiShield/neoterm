...
use uuid::Uuid;
use tokio_stream::{Stream, StreamExt as TokioStreamExt}; // For consuming reqwest response stream
use std::error::Error;
use std::fmt;
...
pub trait AIClient: Send + Sync {
    ...
    async fn chat_completion(&self, messages: Vec<ChatMessage>, tools: Option<serde_json::Value>) -> Result<ChatMessage> {
        Err(anyhow!("Chat completion not implemented for this client"))
    }
    async fn stream_chat_completion(&self, messages: Vec<ChatMessage>, tools: Option<serde_json::Value>) -> Result<mpsc::Receiver<ChatMessage>> {
        Err(anyhow!("Stream chat completion not implemented for this client"))
    }
}
